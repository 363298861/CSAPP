...
//    }else{
//        if(!PREDADR(p) && !SUCCADR(p)){
//            free_listp = NULL;
//        }else if(PREDADR(p)){
//            free_listp = (char*) SUCCADR(p);
//        }else if(SUCCADR(p)){
//            SUCCADR(PREDADR(p)) = 0;
//        }else
//            SUCCADR(PREDADR(p)) = SUCCADR(p);
//    }
}
...
void mm_free(void *ptr)
{
    ...
    printf("The size that is freed is %lu and the position is at %p at line %lu ###################\n", size, ptr, line++);
